...
module.exports =  {
    ...
    async handle(command,args,message)
    {
      ...
      if(command === "tip") {
        ...
        var receiveAddress = await this.mongoInterface.findOne('receive_account',{discordId:recipientId})
        ...
        var success = TransactionHandler.sendTip(tipAccount,receiveAddress,tipAmount)

        if(!success)
        {
          message.channel.send('Tipping failed.. ' );
          return;
        }
